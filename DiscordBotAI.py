...
gpt2.load_gpt2(sess, run_name='run1') # The name of your checkpoint

YOURNAME = "Albert Einstein"
...
async def summarizerprocess_commands(ctx,*,args):
    text = str(args)
                    ...
                    if "makeconvo" in message.content:
                        ...
                        for msg in old:
                            if last_author == msg.author.name:
                                ...
                                prefix = prefix + "\n\n" + msg.author.name + ":\n" + msg.content + "\n"
                        while True:
                            results = gpt2.generate(sess, run_name='run1', temperature=0.9, nsamples=3, batch_size=3, prefix=prefix + "\n\n" + YOURNAME + ":\n", length=250, return_as_list=True, include_prefix=False, truncate="\n\n")
